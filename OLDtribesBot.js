...
bot.once('ready', () => {
  ...
  var d = new Date();
  var n = d.toISOString();
  alertChannel = bot.channels.cache.find(
    (channel) => channel.name === 'bug-reports'
  );
...
bot.on('message', (msg) => {
  ...
  if (msg.channel && msg.channel.name) {
    if (!msg.channel.name.toLowerCase().includes('tribe')) {
      console.log(
        'ignore command >' + msg.content + '<in channel with no tribe '
      );
      return;
    }
  }
...
async function processMessage(msg) {
  ...
  if (msg.channel && msg.channel.name) {
    ...
    if (!gameState) {
      ...
      console.log(
        'loading game ' + msg.channel.name.toLowerCase() + ' from file'
      );
    }
  } else {
  ...
  switch (chanceRoll) {
    ...
    case 14:
      ...
      message +=
        ...
        "'s food [" +
        person.food +
        '], except for grain, spoils and is lost.  ';
      person.food = 0;
    ...
    case 8:
      message +=
        'Favorable weather conditions allow the tribe to make “jerky,” which keeps very well. Each person may trade Food counters for Grain counters (representing the jerky), at a rate of 3 Food for 1 Grain.  Syntax: jerk <amount of food>';
      gameState.canJerky = true;
      break;
    case 7:
      message +=
        'FIRE! The hunting track moves to 20 (no game!) The tribe must move to another area immediately (Section 9). ';
      if (
        util.isColdSeason(gameState) &&
    ...
    case 5:
      ...
      if (person.food < 2) {
        message +=
          '( but they only had ' +
          person.food +
          ' so the ref might kill them if no help is given or grain is used)';
      }
      person.food -= 2;
      if (person.food < 0) {
        person.food = 0;
...
async function handleCommand(msg, author, actor, command, bits, gameState) {
  ...
  if (command == 'addchild') {
    ...
    util.messageChannel(
      'The referee adds a child :' + child.name,
      gameState,
      bot
    );
    return;
  ...
  if (command == 'attack') {
    ...
    if (bits.length != 2 || !gameState.violence) {
      msg.author.send(
        ' usage: !attack <target>, but only after after intimidation has failed'
      );
      cleanUpMessage(msg);
      return;
    }
    ...
    if (targetPlayer.escaped) {
      msg.author.send(
        targetPlayer.name + ' has already run away and can not be attacked.'
      );
      cleanUpMessage(msg);
      return;
    }
  ...
  if (command == 'defend') {
    ...
    if (bits.length != 1 || !gameState.violence) {
      msg.author.send(
        ' usage: !defend, but only after after intimidation has failed'
      );
      cleanUpMessage(msg);
      return;
    }
  ...
  if (command == 'run') {
    ...
    if (bits.length != 1 || !gameState.violence) {
      msg.author.send(
        ' usage: !run, but only after after intimidation has failed'
      );
      cleanUpMessage(msg);
      return;
    }
  ...
  if (command === 'award') {
    if (referees.includes(actor)) {
      ...
      if (msg.mentions.users && msg.mentions.users.first()) {
        targetName = util.removeSpecialChars(
      ...
      if (isNaN(amount) || !types.includes(type)) {
        msg.author.send(
          'award syntax is award <amount> <food|grain|spearhead|basket> <recipient>'
        );
        cleanUpMessage(msg);
        return;
      }
      ...
      util.messageChannel(
        'The game awards ' + targetName + ' with ' + amount + ' ' + type,
        gameState,
        bot
      );
  ...
  if (command == 'babysit') {
    ...
    if (
      babysitter.newAdult ||
      (babysitter.age == 23 && !gameState.reproductionRound)
    ) {
  ...
  if (command == 'chance') {
    ...
    if (gameState.reproductionRound && gameState.needChanceRoll) {
      ...
      msg.author.send(
        command + ' happens after reproduction attempts, before migration'
      );
      return;
    }
  }
  if (command == 'checkmating') {
    ...
    if (referees.includes(actor) || player) {
      // && player.chief
      doneMating = reproLib.globalMatingCheck(gameState, bot);
      msg.author.send(
        'Checking the status of mating; reminding those who need reminding.  ' +
          doneMating +
  ...
  if (command == 'children') {
    ...
    if (bits[1] == '!hungry') {
      ...
      response = childlib.showChildren(
        children,
        population,
        '',
        gameState.secretMating
  ...
  if (command == 'close') {
    ...
    gameState.open = true;
    util.messageChannel(
      'The tribe is only open to those the chief inducts',
      gameState,
      bot
    );
    return;
  ...
  if (command == 'consent') {
    ...
    if (
      gameState &&
      gameState.reproductionList &&
      gameState.reproductionList[0].startsWith(inviterName)
    ) {
  ...
  if (command == 'demand') {
    ...
    util.messageChannel(response, gameState, bot);
    msg.author.send(
      'Your faction will try to intimidate the others, and if that fails, there will be violence.'
    );
    return;
  }
  if (command === 'demote') {
    ...
    if (referees.includes(targetName)) {
      ...
      console.log(
        'did not find the ref in the list, although the include worked.'
      );
      return;
    } else {
  ...
  if (command === 'feed') {
    if (bits.length < 3 || isNaN(bits[1]) || bits[1] <= 0) {
      msg.author.send(
        'feed syntax is feed <amount>  <childname> [<other child names>]'
      );
      cleanUpMessage(msg);
      return;
    }
  ...
  if (command == 'force' || command == 'command') {
    ...
    if (player && (player.chief || target.golem)) {
      if (!target.obeyList || target.obeyList.indexOf(forceCommand) == -1) {
        ...
        if (target.obeyList) {
          msg.author.send(
            target.name + ' would do:' + target.obeyList.join(',')
          );
        }
        return;
      }
    ...
    util.messageChannel(
      actor + ' tells ' + targetName + ' to ' + bits.join(' '),
      gameState,
      bot
    );
    ...
    util.history(
      target.name,
      'Commanded by ' + actor + ' to do something: ' + bits.join(' '),
      gameState
    );
  }
  if (command == 'obey') {
    if (player) {
      ...
      msg.author.send(
        'You will obey if a chief tells you to: ' + player.obeyList
      );
      cleanUpMessage(msg);
      return;
    }
  ...
  if (command === 'give') {
    syntax =
      'Give syntax is give  <amount> <food|grain|spearhead|basket> <recipient>';
    if (bits.length < 3) {
    ...
    if (
      ...
      if (msg) {
        msg.author.send(
          'The game suspects you used that item to work with.  Ask the referee to help trade it if you did not.'
        );
        cleanUpMessage(msg);
      }
      return;
    ...
    if (population[actor][type] >= amount) {
      ...
      util.messageChannel(
        actor + ' gave ' + amount + ' ' + type + ' to ' + targetName,
        gameState,
        bot
      );
      util.history(
        targetName,
        actor + ' gave you ' + amount + ' ' + type,
        gameState
      );
      util.history(
        actor,
        'You gave ' + targetName + '  ' + amount + ' ' + type,
        gameState
      );
      return;
    } else {
      if (msg) {
        if (msg.author) {
          msg.author.send(
            'You do not have that many ' + type + ': ' + population[actor][type]
          );
        }
        cleanUpMessage(msg);
        return;
  ...
  if (command == 'guard' || command == 'watch') {
    ...
    if (person.guarding && person.guarding.length > 4) {
      msg.author.send(
        'You are already guarding enough children: ' + person.guarding
      );
      cleanUpMessage(msg);
      return;
    }
    ...
    if (person.worked == true || gameState.workRound == false) {
      msg.author.send(
        'You can not change guard status after having worked, or outside the work round'
      );
      cleanUpMessage(msg);
      return;
    }
    ...
    while (cName && safety < 30) {
      ...
      if (!child) {
        ...
        util.messageChannel(
          actor + ' starts guarding ' + childName,
          gameState,
          bot
        );
      }
  ...
  if (command == 'ignore') {
    ...
    while (foo) {
      if (
        ...
        util.messageChannel(
          actor + ' stops guarding ' + person.guarding,
          gameState,
          bot
        );
        delete person.guarding;
      ...
      if (!child) {
        ...
        util.messageChannel(
          actor + ' stops guarding ' + childName,
          gameState,
          bot
        );
      }
  ...
  if (command == 'invite') {
    ...
    inviteMessage +=
      actor + ' invites ' + bits[1] + ' to reproduce.  !pass or !consent';
  ...
  if (command.startsWith('jerk')) {
    ...
    response =
      actor + ' converts ' + amount + ' food into ' + output + ' grain';
  ...
  if (command == 'legislate' || command == 'decree') {
    ...
    if (Number(number)) {
      ...
      msg.author.send(
        'syntax: !legislate <number> <everything else is the law>'
      );
    ...
    if (loadedValue && loadedValue.population && loadedValue.population) {
      ...
      if (gameState) {
        ...
        if (gameState.lastSaved) {
          util.messageChannel(
            'All changes since ' + gameState.lastSaved + ' are lost.',
            gameState,
            bot
          );
        }
      ...
      util.messageChannel(
        'Failed to load game from saved file.  This may be a problem.',
        bot
      );
  ...
  if (command == 'migrate') {
    ...
    if (!destination) {
      msg.author.send(
        command +
          ' requires a destination (and force for chief to make it happen)'
      );
      return;
    }
    if (gameState.demand || gameState.violence) {
  ...
  if (command == 'open') {
    ...
    gameState.open = true;
    util.messageChannel(
      'The tribe is open to all who wish to join',
      gameState,
      bot
    );
  }
  if (command == 'pass') {
    ...
    if (inviterName != '') {
      util.messageChannel(
        actor + ' declines a mating invitation from ' + inviterName,
        gameState,
        bot
      );
      delete inviter.invite;
      util.messageChannel(
        inviterName + ' should !invite another partner, or !pass',
        gameState,
        bot
      );
      return;
  ...
  if (command == 'romance') {
    ...
    if (gameState.reproductionRound && gameState.reproductionList) {
      msg.author.send(
        'The mating invitation order is ' + gameState.reproductionList
      );
      for (personName in population) {
        if (population[personName].invite) {
          msg.author.send(
  ...
  if (command === 'sacrifice') {
    syntaxMessage =
      'Sacrifice syntax is sacrifice  <amount> <food|grain|spearhead|basket>';
    if (bits.length < 3) {
  ...
  if (command == 'secretmating') {
    if (referees.includes(actor) || player.chief) {
      if (gameState.reproductionRound) {
        msg.author.send(
          'Wait until the reproduction round is over to change that setting.'
        );
      } else {
  ...
  if (command == 'skip') {
    ...
    if (gameState.secretMating) {
      ...
      if (person) {
        ...
        util.messageChannel(
          'The chief cancels ' + person.name + "'s chance to reproduce",
          gameState,
          bot
        );
    ...
    if (
      gameState.reproductionList &&
      gameState.reproductionList[0] &&
      gameState.reproductionList[0].startsWith(target)
    ) {
    ...
    util.messageChannel(
      'The chief cancels this chance to reproduce',
      gameState,
      bot
    );
    nextMating(actor, gameState);
  ...
  if (
    command == 'startreproduction' ||
    command.startsWith('startr') ||
    command.startsWith('repro') ||
  ...
  if (
    ...
    if (command == 'train') {
      ...
      if (learnRoll >= 10) {
        ...
        message =
          actor +
          ' tries to learn to craft, but does not understand it yet. [' +
          learnRoll +
          ']';
      }
    ...
    if (slackers && slackers.length == 0) {
      util.messageChannel(
        '-= All available workers have worked =-',
        gameState,
        bot
      );
      startFood(gameState, bot);
...
function listReadyToWork(tribe) {
  ...
  for (playerName in tribe) {
    ...
    if (
      person.worked ||
      (person.isInjured && person.isInjured > 0) ||
...
function nextMating(currentInviterName, gameState) {
  ...
  if (!player) {
    console.log(
      'bad attempt to call nextMating, person not found ' + currentInviterName
    );
  }
  for (var targetName in gameState['population']) {
    if (gameState['population'][targetName].invite) {
      console.log(
        'deleting invite from ' +
          targetName +
          ' to ' +
          gameState['population'][targetName].invite
      );
    }
  ...
  if (gameState.reproductionList.length > 0) {
    util.messageChannel(
      gameState.reproductionList[0] +
        ' should now !invite people to reproduce, or !pass ',
      gameState,
      bot
    );
    var cleanName = gameState.reproductionList[0];
    ...
    eligibleMatesMessage = reproLib.eligibleMates(
      cleanName,
      gameState.population
    );
    util.messageChannel(
      'Available partners to invite: ' + eligibleMatesMessage,
      gameState,
      bot
    );
    util.messageChannel(
      'People who have not yet invited: ' +
        gameState.reproductionList.join(', '),
      gameState,
      bot
    );
    ...
    util.messageChannel(
      'Reproduction round is over.  Time for the chance roll',
      gameState,
      bot
    );
    return;
...
function spawnFunction(
  ...
  if (
    population[mother].isPregnant &&
    population[mother].isPregnant != '' &&
    population[mother].isPregnant != 'false'
  ) {
  ...
  if (force != false || mroll + droll >= spawnChance) {
    ...
    if (gameState.reproductionList) {
      ...
      if (hasNotMated) {
        console.log(
          'attempting to remove pregnant woman ' +
            mother +
            ' from the reproduction list'
        );
        gameState.reproductionList.delete(
          gameState.reproductionList.indexOf(mother)
        );
      } else {
  ...
  if (allPregnant) {
    ...
    util.messageChannel(
      'No more reproductive pairings are available.',
      gameState,
      bot
    );
...
function consumeFoodPlayers(gameState) {
  ...
  for (var target in population) {
    ...
    console.log(
      target +
        ' f:' +
        population[target].food +
        ' g:' +
        population[target].grain
    );
    ...
    if (population[target].food < 0) {
      ...
      population[target].grain =
        population[target].grain + population[target].food;
    ...
    if (
      population[target].gender == 'female' &&
      countChildrenOfParentUnderAge(children, target, 4) > 1
    ) {
      ...
      if (population[target].food < 0) {
        ...
        if (population[target].grain < 0) {
          ...
          response +=
            target + ' lost her child ' + child + ' due to lack of food\n';
  ...
  for (childName in children) {
    ...
    if (child.age < 24) {
      ...
      if (
        ...
        if (
          population[child.mother].nursing &&
          population[child.mother].nursing.length == 0
        ) {
...
function migrate(msg, destination, force, gameState) {
  ...
  if (force) {
    ...
    util.messageChannel(
      'Setting the current location to ' + destination,
      gameState,
      bot
    );
    ...
    response +=
      '\nThe following tribe members would die on the journey to ' +
      destination +
      ': ' +
      deceasedPeople;
    response +=
