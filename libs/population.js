...
function memberByName(name, gameState) {
  ...
  if (gameState == null) {
    console.log(
      'tried to get member when gameState was null.  probably a syntax error'
    );
    return null;
  }
...
function memberByNameFromDictionary(name, dictionary) {
  ...
  if (dictionary == null) {
    console.log(
      'tried to get member when dictionary was null.  probably a syntax error'
    );
    return null;
  }
  cleaned = text.removeSpecialChars(name);
...
function history(playerName, message, gameState) {
  ...
  if (!player) {
    console.log(
      'trying to record history with no player record:' +
        playerName +
        ': ' +
        message
    );
  ...
  if (!player) {
    ...
    if (player) {
      text.addMessage(
        gameState,
        playerName,
        'Before you left the tribe, these things happened:'
      );
    } else {
      text.addMessage(
        gameState,
...
function addToPopulation(gameState, sourceName, gender, profession, handle) {
  ...
  if (sourceName != target) {
    text.addMessage(
      gameState,
      target,
      'Names with non-alphanumeric characters may impair game function.\n'
    );
  }
  ...
  console.log(
    'added ' + target + ' ' + gender + ' to the tribe. strRoll:' + strRoll
  );
...
function vote(gameState, actorName, candidateName) {
  ...
  if (!candidate) {
    text.addMessage(
      gameState,
      actorName,
      candidateName + ' not found in the tribe'
    );
    return;
  }
  if (!player) {
  ...
  text.addMessage(
    gameState,
    'tribe',
    player.name + ' supports ' + candidate.name + ' as chief'
  );
