...
function command(gameState, actorName, targetName, action, extraArg){
    ...
    pop.history(targetMember.name,actingMember.name+" ordered you to "+action, gameState);
    ...
    if (action == "hunt"){
            ...
            if (targetMember.canCraft && (extraArg == "spearhead" || extraArg == "basket")){
                worklib.craft(gameState, targetMember.name, extraArg, 0 );
            } else {
                text.addMessage(gameState, actingMember.name, targetMember.name +" does not know how to do that.");
            }
            return;
    }
