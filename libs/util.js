...
function gameStateMessage(gameState) {
  ...
  var message =
    'It is Year ' + gameState.seasonCounter / 2 + ', ' + season + '.\n';
  message +=
    'The ' +
    gameState.name +
    ' tribe has ' +
  ...
  if (gameState.workRound) {
    message += 'It is the work round.';
  }
  if (gameState.foodRound) {
    message += 'It is the food round.';
  }
  if (gameState.reproductionRound) {
    const pendingInviteCount = reproLib.canStillInviteCount(gameState);
    if (pendingInviteCount > 0) {
      message +=
        'It is the reproduction round: awaiting invitations or pass from ' +
        pendingInviteCount +
        ' player' +
        (pendingInviteCount === 1 ? '' : 's') +
        '.';
    } else if (gameState.needChanceRoll) {
      message += 'It is the reproduction round, awaiting chance.';
    } else {
      message += 'It is the reproduction round, awaiting migration or not.';
    }
  } else {
