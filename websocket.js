  ...
  ws.on('message', (message) => {
    try {
      ...
      if ('tribe' in data) {
        tribe = data.tribe;
        tribePath =
          '../tribesAgent/' + tribe + '-tribe/' + tribe + '-tribe.json';
      }
        ...
        if (command === 'give' && args.length === 4) {
          ...
          ws.send(
            JSON.stringify({
              ...
              command: 'give',
              success:
                !lastMessage.message.includes('not found') &&
                !lastMessage.message.includes('does not have'),
          ...
          ws.send(
            JSON.stringify({
              type: 'error',
              message:
                'Invalid command or arguments. Expected: give <source> <target> <amount> <item>',
              clientId,
            })
          );
        }
...
function processRomance(data, tribeData) {
  ...
  if (userData) {
    if (inviteList && !_array_match(inviteList, userData['inviteList'])) {
      ...
      console.log(
        'inviteList:' + tribeData['population'][name]['inviteList'].join()
      );
    }
...
function validateUser(userData) {
  ...
  for (const [existingEmail, userRecord] of Object.entries(usersDict)) {
    if (
      existingEmail == email &&
      !verifyPassword(password, userRecord.password)
    ) {
...
async function registerUser(userData) {
  ...
  var email = userData.email;
  var password = '';
  hashPassword(userData.password).then((hashedValue) => {
