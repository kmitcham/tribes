    ...
    describe('Complex Workflow Scenarios', () => {
        test('should handle complete reproduction workflow', async () => {
            const commandSequence = [
                ...
                { command: 'invite', parameters: { invitelist: ['PreferredMate', 'BackupChoice'] } },
            ...
            expect(responses[1].sent.parameters.invitelist).toEqual(['PreferredMate', 'BackupChoice']);
            expect(responses[1].received.success).toBe(true);
            expect(responses[1].received.message).toContain('PreferredMate, BackupChoice');
        ...
        test('should handle complete work workflow', async () => {
            ...
            expect(responses[0].sent.parameters).toEqual({ item: 'spear', force: '3' });
            expect(responses[0].received.success).toBe(true);
        ...
        test('should maintain client state during command sequence', async () => {
            ...
            for (const command of commands) {
                client.executeCommand(command, command === 'craft' ? { item: 'basket' } : command === 'guard' ? { player: 'TestTarget' } : {});
                
                const sentMessage = JSON.parse(client.ws.sentMessages[client.ws.sentMessages.length - 1]);
            ...
            expect(allMessages.every(msg => msg.playerName === 'TestPlayer')).toBe(true);
        });
    });
    ...
    describe('Parameter Format Validation', () => {
        test('should preserve parameter types in transmission', async () => {
            ...
            for (const testCase of testCases) {
                ...
                for (const [key, expectedType] of Object.entries(testCase.expectedTypes)) {
                    expect(typeof sentParameters[key]).toBe(expectedType);
                    if (expectedType === 'object' && Array.isArray(testCase.parameters[key])) {
