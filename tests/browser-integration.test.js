test('guard and ignore work for player name casing (Kevin, kevin, KEVIN)', async () => {
  ...
  for (const casing of ['Kevin', 'kevin', 'KEVIN']) {
    ...
    const guardModalText = await page.$eval(
      '#commandModal',
      (el) => el.innerText
    );
    ...
    const hasPebble =
      (await page.$('input[name="unified_guard_Pebble"][value="guard"]')) !==
      null;
    if (hasPebble) {
      ...
      expect(msg).toMatch(
        /Kevin starts guarding Pebble|kevin starts guarding Pebble|KEVIN starts guarding Pebble/i
      );
    }

    await openCommand('ignore');
    // Should show the unified guard UI and not error
    const ignoreModalText = await page.$eval(
      '#commandModal',
      (el) => el.innerText
    );
...
const TEST_TRIBE_DIR = path.resolve(
  __dirname,
  '..',
  'tribe-data',
  TEST_TRIBE_NAME
);
const TEST_TRIBE_STATE_PATH = path.join(
  TEST_TRIBE_DIR,
  `${TEST_TRIBE_NAME}.json`
);
const TEST_TRIBE_FIXTURE_PATH = path.resolve(
  __dirname,
  'browser-integration-tribe.json'
);
const TRIBES_REGISTRY_PATH = path.resolve(
  __dirname,
  '..',
  'tribe-data',
  'tribesRegistry.json'
...
async function getPlayerGuarding(playerName) {
  return page.evaluate((currentPlayerName) => {
    ...
    const player =
      ...
      Object.values(population).find(
        (person) => person.name === currentPlayerName
      );
...
describe('Browser Integration: tribes-interface.html', () => {
  test('status update renders conflict banner for demand and violence', async () => {
    ...
    let bannerState = await page.evaluate(() => ({
      ...
      isViolence: document
        ...
        .classList.contains('violence'),
      topConflictText:
        document.getElementById('conflictStatus')?.innerText || '',
    }));
    ...
    expect(bannerState.body).toContain(
      'still need to pick attack, run away, or defend before the round can advance.'
    );
    expect(bannerState.body).toContain(
    ...
    expect(bannerState.body).toContain(
      escapedPlayerName.charAt(0).toUpperCase() + escapedPlayerName.slice(1)
    );
    ...
    const escapedCardShowsRunAway = await page.evaluate((escapedName) => {
      ...
      const badgeTexts = Array.from(
        targetCard.querySelectorAll('.person-badge')
  ...
  test('romance modal submits invite and consent updates together', async () => {
    ...
    await page.evaluate(() => {
      ...
      client.selectedCommand = {
        name: 'romance',
        description: 'Romance modal test',
      };
    ...
    const result = await page.evaluate(() => {
      ...
      const romancePayload = sentPayloads.find(
        (p) => p.type === 'romanceRequest'
      );
    ...
    expect(result.invitePayload.parameters.invitelist).toContain(
      result.firstName
    );
  ...
  test('connect and execute craft command', async () => {
    ...
    const modalClasses = await page.evaluate(
      () => document.getElementById('commandModal').className
    );
    ...
    expect(messages).toContain('You are not a member of this tribe. Join if the tribe is open, or ask the chief to induct you.');
  }, 15000);
  ...
  test('guard and ignore update the guarded child list', async () => {
    ...
    await page.waitForSelector(
      'input[name="unified_guard_Pebble"][value="guard"]'
    );
    await page.click('input[name="unified_guard_Pebble"][value="guard"]');
    await page.click('#modalExecuteBtn');
    ...
    await openCommand('ignore');
    await page.waitForSelector(
      'input[name="unified_guard_Pebble"][value="ignore"]'
    );
    expect(
      await page.$eval(
        'input[name="unified_guard_Pebble"][value="guard"]',
  ...
  test('give command exits loading state when population data arrives', async () => {
    ...
    await page.waitForFunction(() => {
      ...
      const loading = params.innerText
        .toLowerCase()
        .includes('loading player data');
