...
beforeEach(() => {
  gameState = {
    ...
    population: {
      Bob: { food: 2, grain: 0, isInjured: 1, name: 'Bob' },
...
test('should return "not reproduction round" if not in reproduction round', () => {
  ...
  expect(m.migrate('Bob', 'hills', false, gameState)).toBe(
    'not reproduction round'
  );
});
...
test('migration with force set to false', () => {
  ...
  expect(text.addMessage).toHaveBeenCalledWith(
    ...
    expect.stringContaining(
      'The following tribe members would die on the journey to hills'
    )
  );
  expect(text.addMessage).toHaveBeenCalledWith(
    expect.anything(),
    sourceName,
    expect.stringContaining('The following children would die along the way')
  );
