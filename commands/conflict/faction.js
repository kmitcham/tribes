...
function setFaction(gameState, actorName, side){
    ...
    if (gameState['violence']){
        text.addMessage(gameState, actorName, 'The time for factions has past; it has come to violence.  You must attack, defend or run.')
        return
    }
    if (!gameState['demand']){
        text.addMessage(gameState, actorName, 'There is no demand to have a faction about')
        return
    }
    player.faction = side;
