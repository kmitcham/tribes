...
// send messages to discord channel and users
async function sendMessages(bot, gameState, interaction) {
  ...
  const channel = bot.channels.cache.find(
    (channel) => channel.name === gameState.name
  );
  if (!messagesDict) {
  ...
  if (actorName in messagesDict && needsReply) {
    ...
    if (!message) {
      ...
      if (!chunks || chunks.length == 0) {
        ...
        chunks = message.match(/[\S\s]{1,1900}/g);
        console.log(
          'message needs reply, with this many chunks:' + chunks.length
        );
